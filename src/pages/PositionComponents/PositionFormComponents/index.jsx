...
const PositionForm = ({onSubmit, Position}) => {
    ...
    const validate = () => {
        ...
        if (!formData.positionID) {
            newErrors.positionName = 'id Chức vụ không được để trống';
        } else if (formData.positionID.length !== 10) {
            newErrors.positionID= 'id Chức vụ là 10 ký tự';
        } else if (Position.some(Position => Position.positionID === formData.positionID)) {
            newErrors.positionID = 'id Chức vụ đã tồn tại';
        }

        if (!formData.departmentType) newErrors.departmentType = 'Loại tên phòng ban không được để trống';
